...
def _compute_reachability(
    logprobs: list, baseline_logprobs: Optional[list] = None
) -> float:
    if not logprobs:
        return 0.5
    ...
    def get_avg_logprob(lps: list) -> float:
        valid_lps = [
            t.get("logprob", -10.0) for t in lps if t.get("logprob") is not None
        ]
        if not valid_lps:
            return -10.0
...
def score(
    ...
    cache_state = json.dumps(
        {"result": result.text, "task": task, "expected_output": expected_output}
    )
